...
| :--- | :--- | :--- |
| **기능 결합** | 공통 로직을 부모 클래스에 집중시켜 관리가 편하지만, 원치 않는 기능도 상속받게 됨. | 필요한 기능(능력 실행, 스킬 사용 등)만 인터페이스로 구현하여 객체별 맞춤 설계 가능. |
| **코드 재사용** | 상속 계층구조 내에서만 코드 재사용이 가능함. | 인터페이스 규격만 맞추면 어떤 클래스에서든 해당 기능을 재사용할 수 있음. |
| **유지보수** | 부모 클래스 변경 시 하위 모든 클래스의 영향도를 파악해야 함. | 인터페이스 단위로 기능이 분리되어 있어 특정 기능 수정이 다른 기능에 미치는 영향이 적음. |

*   **선택 이유:** **B안(인터페이스 기반)**의 장점을 취하면서도, 공통 데이터 관리를 위해 **상속(Base Class)을 병행**하는 절충안을 선택했습니다. `IAbility` 인터페이스로 실행 규격을 정의하고, `AbilityBase` 추상 클래스로 공통 속성을 관리함으로써 유연성과 생산성을 동시에 확보했습니다.
